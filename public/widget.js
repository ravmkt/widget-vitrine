...
function readStoreSettings() {
    if (!storeId || !hasSupabase) return Promise.resolve({});
    return supabaseFetch(
'store_settings_public?select=auto_approve_comments,whatsapp_number,whatsapp_message,whatsapp_message_template,whatsapp_enabled,store_name,logo_url,live_widget_config,live_player_config&store_id=eq.' + encodeURIComponent(storeId) + '&limit=1',
      { method: 'GET' }
    )
      .then(function (response) { if (!response.ok) return {}; return response.json(); })
      .then(function (data) {
...
+ '.vl-modal{position:relative!important;width:100%!important;max-width:420px!important;height:100%!important;max-height:100dvh!important;background:#000!important;overflow:hidden!important;display:flex!important;flex-direction:column!important;border:' + modalBorderWidthNum + 'px solid ' + modalBorderColor + '!important;box-shadow:' + shadow + '!important;}'
      ...
      /* Sobe os botões da lateral direita para livrar espaço total do card */
      + '.vl-sidebar-actions{position:absolute!important;right:12px!important;bottom:185px!important;z-index:45!important;display:flex!important;flex-direction:column!important;gap:12px!important;align-items:center!important;}'
      /* Card colado nos botões inferiores com exatamente 3px de margem */
      + '.vl-product{display:flex!important;align-items:center!important;gap:10px!important;border-radius:' + modalConfig.product_card_border_radius + 'px!important;border:' + modalConfig.product_card_border_width + 'px solid ' + modalConfig.product_card_border_color + '!important;padding:8px 12px!important;background:' + modalConfig.product_card_bg + '!important;cursor:pointer!important;box-shadow:' + shadow + '!important;pointer-events:auto!important;margin-bottom:3px!important;}'
      ...
      + '.vl-product-info{min-width:0!important;flex:1!important;display:flex!important;flex-direction:column!important;justify-content:center!important;gap:1px!important;}'
      + '.vl-product-name{font-weight:700!important;font-size:' + modalConfig.product_card_name_size + 'px!important;color:' + modalConfig.product_card_name_color + '!important;display:-webkit-box!important;-webkit-line-clamp:2!important;-webkit-box-orient:vertical!important;overflow:hidden!important;word-break:break-word!important;line-height:1.25!important;white-space:normal!important;}'
      + '.vl-product-price{margin-top:1px!important;font-weight:800!important;font-size:' + modalConfig.product_card_price_size + 'px!important;color:' + modalConfig.product_card_price_color + '!important;}'
      + '.vl-product-actions{display:flex!important;align-items:center!important;gap:8px!important;width:100%!important;box-sizing:border-box!important;pointer-events:auto!important;}'
      + '.vl-product-btn{all:unset!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;gap:6px!important;box-sizing:border-box!important;flex:1 1 0px!important;min-width:0!important;height:42px!important;border-radius:12px!important;padding:0 12px!important;background:' + modalConfig.product_card_button_bg + '!important;color:' + modalConfig.product_card_button_color + '!important;font-size:13px!important;font-weight:700!important;cursor:pointer!important;text-decoration:none!important;text-align:center!important;pointer-events:auto!important;white-space:nowrap!important;transition:opacity .2s!important;}'
      + '.vl-product-btn:hover{opacity:.9!important;}'
      + '.vl-product-whatsapp-btn{all:unset!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;gap:6px!important;box-sizing:border-box!important;flex:1 1 0px!important;min-width:0!important;height:42px!important;border-radius:12px!important;padding:0 12px!important;background:#25d366!important;color:#fff!important;font-size:13px!important;font-weight:700!important;cursor:pointer!important;text-decoration:none!important;text-align:center!important;pointer-events:auto!important;white-space:nowrap!important;transition:background .2s!important;}'
...
// 1. YouTube Shorts ou Vídeo Tradicional (Respeita a preferência de áudio ativa do usuário)
    ...
    if (url) {
      ...
      media.loop = false; // Corrigido para disparar evento 'ended' e avancar sequencialmente
...
if (showVerProduto) {
  ...
  if (productData) {
    ...
    if (storeWhatsappEnabled && storeWhatsappNumber) {
      ...
      // Ícone Oficial do WhatsApp com telefone recortado visível
      var waIconSvg = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" style="flex-shrink:0;"><path fill-rule="evenodd" clip-rule="evenodd" d="M12.04 2C6.58 2 2.13 6.45 2.13 11.91C2.13 13.66 2.59 15.36 3.45 16.86L2.05 22L7.3 20.63C8.75 21.41 10.38 21.83 12.04 21.83C17.5 21.83 21.95 17.38 21.95 11.92C21.95 9.27 20.92 6.78 19.05 4.91C17.18 3.03 14.69 2 12.04 2ZM12.04 20.15C10.56 20.15 9.11 19.76 7.85 19.01L7.55 18.83L4.43 19.65L5.26 16.61L5.06 16.29C4.24 14.99 3.81 13.47 3.81 11.91C3.81 7.37 7.5 3.68 12.04 3.68C14.25 3.68 16.31 4.54 17.86 6.1C19.42 7.66 20.28 9.72 20.28 11.92C20.28 16.46 16.58 20.15 12.04 20.15ZM16.56 14.37C16.31 14.25 15.09 13.65 14.86 13.57C14.64 13.48 14.47 13.44 14.31 13.69C14.14 13.94 13.66 14.5 13.51 14.67C13.37 14.84 13.22 14.86 12.97 14.74C12.72 14.61 11.92 14.35 10.97 13.5C10.23 12.84 9.73 12.03 9.58 11.78C9.44 11.53 9.57 11.39 9.69 11.27C9.8 11.16 9.94 10.98 10.06 10.83C10.19 10.69 10.23 10.58 10.31 10.42C10.39 10.25 10.35 10.11 10.29 9.98C10.23 9.86 9.73 8.63 9.53 8.13C9.32 7.64 9.12 7.71 8.96 7.7C8.82 7.69 8.65 7.69 8.48 7.69C8.32 7.69 8.05 7.75 7.82 8C7.6 8.25 6.96 8.85 6.96 10.07C6.96 11.29 7.85 12.46 7.97 12.63C8.1 12.79 9.71 15.28 12.18 16.35C12.77 16.6 13.23 16.76 13.59 16.87C14.18 17.06 14.72 17.03 15.15 16.97C15.63 16.9 16.62 16.37 16.83 15.79C17.03 15.21 17.03 14.71 16.97 14.61C16.91 14.51 16.81 14.49 16.56 14.37Z" fill="white"/></svg>';

      waStoreBtn.innerHTML = waIconSvg + '<span>WhatsApp</span>';
...
function initWidget() {
  ...
  // 🚀 Modo preview de simulação (botão "Simular Preview" do painel)
  var _previewStoryId = _urlParams.get('vidlytics_preview_story_id') || null;
      ...
      return readDisplayLocations().then(function (locations) {
        return readPageRules().then(function (rules) {
          ...
          activeLocations.forEach(function (location) {
            ...
            // 🚀 Modo preview: ignora as regras de página para a story simulada aparecer em qualquer URL
            if (storyRules.length > 0 && !_previewStoryId) {
            ...
            try {
              ...
              // 🔧 Só marca como injetado se o widget realmente renderizou (seletor encontrado na página)
              if (widgetInjected) {
                injected = true;
          ...
          // ────────────────────────────────────────────
          // 🔧 FALLBACK — stories inline que não foram injetadas em nenhum seletor
          // ────────────────────────────────────────────
