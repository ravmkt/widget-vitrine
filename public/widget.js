...
(function () {
  ...
  function readAppearanceValue(appearance, names) {
    ...
    for (var i = 0; i < names.length; i += 1) {
      if (appearance[names[i]] !== undefined && appearance[names[i]] !== null && appearance[names[i]] !== '') {
    ...
    for (var k = 0; k < keys.length; k += 1) {
      ...
      if (normalizedNames.indexOf(normalizeKey(currentKey)) !== -1) {
        if (appearance[currentKey] !== undefined && appearance[currentKey] !== null && appearance[currentKey] !== '') {
  ...
  function getConfigAppearance() {
    ...
    [
      ...
      config.floatingVideoAppearance, config.floating_video, widgetsCfg.appearance, widgetsCfg.aparencia,
      widgetsCfg.appearanceConfig, widgetsCfg.appearance_config, widgetsCfg.floating, widgetsCfg.floatingConfig,
      widgetsCfg.floatingAppearance, widgetsCfg.floatingVideoConfig, widgetsCfg.floatingVideoAppearance,
  ...
  function getStorageAppearance() {
    var merged = {};
    var keys = [
      'vidlytics_appearance', 'vidlytics_appearance_' + storeId, 'vidlytics_aparencia',
      'vidlytics_aparencia_' + storeId, 'vidlytics_widget_appearance', 'vidlytics_widget_appearance_' + storeId,
      'vidlytics_config', 'vidlytics_config_' + storeId, 'VIDLYTICS_APPEARANCE', 'VIDLYTICS_CONFIG'
    ];
  ...
  function appearanceHasUsefulData(appearance) {
    appearance = normalizeAppearanceItem(appearance || {});
    var usefulNames = [
      'floating_position', 'floatingPosition', 'position', 'posicao', 'posição', 'widget_position', 'widgetPosition',
      'placement', 'floating_video_position', 'floatingVideoPosition', 'floating_shape', 'floatingShape', 'shape',
      'form', 'forma', 'formato', 'widget_shape', 'widgetShape', 'floating_video_shape', 'floatingVideoShape',
      'floating_width', 'floatingWidth', 'width', 'largura', 'widget_width', 'widgetWidth', 'floating_video_width',
      'floatingVideoWidth', 'floating_height', 'floatingHeight', 'height', 'altura', 'widget_height', 'widgetHeight',
      'floating_video_height', 'floatingVideoHeight', 'floating_radius', 'floatingRadius', 'border_radius',
      'borderRadius', 'radius', 'raio', 'widget_radius', 'widgetRadius', 'floating_top', 'floatingTop', 'top',
      'top_spacing', 'topSpacing', 'spacing_top', 'spacingTop', 'floating_bottom', 'floatingBottom', 'bottom',
      'bottom_spacing', 'bottomSpacing', 'spacing_bottom', 'spacingBottom', 'floating_side', 'floatingSide', 'side',
      'left_spacing', 'leftSpacing', 'right_spacing', 'rightSpacing', 'distance_top', 'distanceTop', 'distancia_superior',
      'distanciaSuperior', 'distance_bottom', 'distanceBottom', 'distancia_inferior', 'distanciaInferior', 'distance_side',
      'distanceSide', 'distancia_lateral', 'distanciaLateral', 'floating_border_width', 'floatingBorderWidth',
      'border_width', 'borderWidth', 'largura_borda', 'larguraBorda', 'primary_color', 'primaryColor', 'secondary_color',
      'secondaryColor', 'border_color', 'borderColor', 'color', 'text_color', 'textColor', 'font_family', 'fontFamily',
      'background_color', 'backgroundColor', 'button_color', 'buttonColor', 'show_title', 'showTitle', 'show_product',
      'showProduct', 'hide_stories', 'hideStories', 'shadow_enabled', 'shadowEnabled', 'floating_config', 'floatingConfig',
      'floating_border_radius', 'floatingBorderRadius', 'widget_border_radius', 'widgetBorderRadius', 'widget_radius',
      'widgetRadius', 'border_radius', 'borderRadius', 'floating_object_fit', 'floatingObjectFit', 'object_fit',
      'objectFit', 'image_fit', 'imageFit', 'fit', 'show_play_button', 'showPlayButton', 'show_player_button',
      'showPlayerButton', 'play_button_enabled', 'playButtonEnabled', 'allow_drag', 'allowDrag', 'draggable',
      'drag_enabled', 'dragEnabled', 'permitir_arrastar', 'allow_close', 'allowClose', 'closable', 'close_enabled',
      'closeEnabled', 'show_close_button', 'showCloseButton', 'permitir_fechar', 'floating_show_play_button',
      'floatingShowPlayButton', 'show_floating_play_button', 'showFloatingPlayButton', 'allow_floating_drag',
      'allowFloatingDrag', 'floating_allow_drag', 'floatingAllowDrag', 'floating_drag_enabled', 'floatingDragEnabled',
      'allow_floating_close', 'allowFloatingClose', 'floating_allow_close', 'floatingAllowClose',
      'floating_close_enabled', 'floatingCloseEnabled'
    ];
  ...
  function extractAppearanceFromItem(item, allowDirectFields) {
    ...
    var merged = {};
    [
      item.appearance, item.aparencia, item.appearance_config, item.appearanceConfig, item.widget_appearance,
      item.widgetAppearance, item.widget_config, item.widgetConfig, item.settings, item.config, item.style,
      item.styles, item.data, item.metadata, item.customization, item.customization_config, item.theme,
      item.theme_config, item.floating, item.floating_config, item.floatingConfig, item.floatingAppearance,
      item.floating_video, item.floatingVideo, item.floatingVideoConfig, item.floatingVideoAppearance
    ].forEach(function (src) {
      flattenAppearanceInto(merged, src, 0);
    });
    ...
    if (allowDirectFields) {
      ...
      var directRadius = firstDefined(
        item.floating_radius, item.floatingRadius, item.floating_border_radius, item.floatingBorderRadius,
        item.widget_radius, item.widgetRadius, item.widget_border_radius, item.widgetBorderRadius,
        item.border_radius, item.borderRadius, item.radius, item.raio
      );
      ...
      var directBorderColor = firstDefined(
        item.floating_border_color, item.floatingBorderColor, item.floating_video_border_color,
        item.floatingVideoBorderColor, item.widget_border_color, item.widgetBorderColor,
        item.border_color, item.borderColor, item.cor_borda, item.corDaBorda, item.cor_da_borda
      );
      ...
      var directObjectFit = firstDefined(
        item.floating_object_fit, item.floatingObjectFit, item.object_fit, item.objectFit,
        item.image_fit, item.imageFit, item.media_fit, item.mediaFit, item.thumbnail_fit,
        item.thumbnailFit, item.fit
      );
      ...
      var directShowPlayButton = firstDefined(
        item.show_play_button, item.showPlayButton, item.show_player_button, item.showPlayerButton,
        item.play_button_enabled, item.playButtonEnabled, item.floating_play_button, item.floatingPlayButton,
        item.floating_show_play_button, item.floatingShowPlayButton, item.show_floating_play_button,
        item.showFloatingPlayButton, item.player_button, item.playerButton, item.mostrar_play,
        item.mostrarPlay, item.mostrar_botao_play, item.mostrarBotaoPlay
      );
      ...
      var directAllowDrag = firstDefined(
        item.allow_drag, item.allowDrag, item.draggable, item.drag_enabled, item.dragEnabled,
        item.floating_draggable, item.floatingDraggable, item.allow_floating_drag, item.allowFloatingDrag,
        item.floating_allow_drag, item.floatingAllowDrag, item.floating_drag_enabled, item.floatingDragEnabled,
        item.permitir_arrastar, item.permitirArrastar, item.arrastar, item.arrastavel
      );
      ...
      var directAllowClose = firstDefined(
        item.allow_close, item.allowClose, item.closable, item.close_enabled, item.closeEnabled,
        item.show_close_button, item.showCloseButton, item.floating_close_button, item.floatingCloseButton,
        item.allow_floating_close, item.allowFloatingClose, item.floating_allow_close, item.floatingAllowClose,
        item.floating_close_enabled, item.floatingCloseEnabled, item.permitir_fechar, item.permitirFechar,
        item.fechar, item.fechavel
      );
    ...
    var floatingConfig = parseJsonIfNeeded(
      firstDefined(
        item.floating_config, item.floatingConfig, item.config && item.config.floating_config,
        item.config && item.config.floatingConfig, item.settings && item.settings.floating_config,
  ...
  function normalizeFloatingPosition(value) {
    ...
    if (key === 'fixed-top-left' || key === 'top-left' || key === 'left-top' || key === 'superior-esquerda' || key === 'canto-superior-esquerdo') return 'top-left';
    if (key === 'fixed-top-right' || key === 'top-right' || key === 'right-top' || key === 'superior-direita' || key === 'canto-superior-direito') return 'top-right';
    if (key === 'fixed-bottom-left' || key === 'bottom-left' || key === 'left' || key === 'left-bottom' || key === 'inferior-esquerda' || key === 'canto-inferior-esquerdo') return 'bottom-left';
    if (key === 'fixed-bottom-right' || key === 'bottom-right' || key === 'right' || key === 'right-bottom' || key === 'inferior-direita' || key === 'canto-inferior-direito') return 'bottom-right';
    return DEFAULT_APPEARANCE.floating_position;
  }

  function normalizeFloatingShape(value) {
    var key = normalizeKey(value);
    if (key === 'square' || key === 'quadrado' || key === 'rectangle' || key === 'retangular') return 'square';
    if (key === 'portrait' || key === 'retrato' || key === 'vertical' || key === '9-16' || key === '9:16') return 'portrait';
    if (key === 'circle' || key === 'circulo' || key === 'circular' || key === 'redondo') return 'circle';
  ...
  function toNumber(value, fallback) {
    ...
    var normalized = String(value).trim().replace('px', '').replace(',', '.');
  ...
  function px(value, fallback) {
    ...
    if (typeof value === 'string') {
      var trimmed = value.trim();
      if (trimmed === 'auto' || trimmed.indexOf('px') !== -1 || trimmed.indexOf('%') !== -1 || trimmed.indexOf('vh') !== -1 || trimmed.indexOf('vw') !== -1 || trimmed.indexOf('rem') !== -1 || trimmed.indexOf('em') !== -1) return trimmed;
    }
  ...
  function getFloatingConfig(appearance) {
    ...
    var positionRaw = getValue(['floating_position', 'floatingPosition', 'position', 'posicao', 'posição', 'widget_position', 'widgetPosition', 'placement', 'floating_video_position', 'floatingVideoPosition'], DEFAULT_APPEARANCE.floating_position);
    var shapeRaw = getValue(['floating_shape', 'floatingShape', 'shape', 'form', 'forma', 'formato', 'widget_shape', 'widgetShape', 'floating_video_shape', 'floatingVideoShape'], DEFAULT_APPEARANCE.floating_shape);
    ...
    var widthNumber = toNumber(getValue(['floating_width', 'floatingWidth', 'width', 'largura', 'widget_width', 'widgetWidth', 'floating_video_width', 'floatingVideoWidth'], defaultWidth), defaultWidth);
    var heightNumber = toNumber(getValue(['floating_height', 'floatingHeight', 'height', 'altura', 'widget_height', 'widgetHeight', 'floating_video_height', 'floatingVideoHeight'], defaultHeight), defaultHeight);

    if (shape === 'square' || shape === 'circle') heightNumber = widthNumber;

    var borderWidthNumber = toNumber(getValue(['floating_border_width', 'floatingBorderWidth', 'border_width', 'borderWidth', 'largura_borda', 'larguraBorda'], DEFAULT_APPEARANCE.floating_border_width), DEFAULT_APPEARANCE.floating_border_width);
    var radiusValue = getValue(['floating_border_radius', 'floatingBorderRadius', 'floating_radius', 'floatingRadius', 'widget_border_radius', 'widgetBorderRadius', 'widget_radius', 'widgetRadius', 'border_radius', 'borderRadius', 'radius', 'raio'], DEFAULT_APPEARANCE.floating_border_radius);
    var radiusNumber = toNumber(radiusValue, DEFAULT_APPEARANCE.floating_border_radius);
    if (shape === 'circle') radiusNumber = 999;

    var topNumber = toNumber(getValue(['floating_top', 'floatingTop', 'top', 'top_spacing', 'topSpacing', 'spacing_top', 'spacingTop', 'distance_top', 'distanceTop', 'distancia_superior', 'distanciaSuperior'], DEFAULT_APPEARANCE.floating_top), DEFAULT_APPEARANCE.floating_top);
    var bottomNumber = toNumber(getValue(['floating_bottom', 'floatingBottom', 'bottom', 'bottom_spacing', 'bottomSpacing', 'spacing_bottom', 'spacingBottom', 'distance_bottom', 'distanceBottom', 'distancia_inferior', 'distanciaInferior'], DEFAULT_APPEARANCE.floating_bottom), DEFAULT_APPEARANCE.floating_bottom);
    var sideNumber = toNumber(getValue(['floating_side', 'floatingSide', 'side', 'left_spacing', 'leftSpacing', 'right_spacing', 'rightSpacing', 'distance_side', 'distanceSide', 'distancia_lateral', 'distanciaLateral'], DEFAULT_APPEARANCE.floating_side), DEFAULT_APPEARANCE.floating_side);
    var zIndexNumber = toNumber(getValue(['z_index', 'zIndex', 'z-index', 'floating_z_index', 'floatingZIndex'], DEFAULT_APPEARANCE.z_index), DEFAULT_APPEARANCE.z_index);

    var objectFitRaw = getValue(['floating_object_fit', 'floatingObjectFit', 'object_fit', 'objectFit', 'fit', 'video_fit', 'videoFit'], DEFAULT_APPEARANCE.floating_object_fit);
    var objectFit = String(objectFitRaw || 'cover').trim().toLowerCase().replace(/_/g, '-');
    if (['cover', 'contain', 'fill', 'none', 'scale-down'].indexOf(objectFit) === -1) objectFit = 'cover';

    var showPlayButton = toBoolean(getValue(['show_play_button', 'showPlayButton', 'floating_show_play_button', 'floatingShowPlayButton', 'play_button', 'playButton', 'mostrar_botao_play', 'mostrarBotaoPlay', 'mostrar_play'], DEFAULT_APPEARANCE.show_play_button), DEFAULT_APPEARANCE.show_play_button);
    var allowDrag = toBoolean(getValue(['allow_drag', 'allowDrag', 'floating_allow_drag', 'floatingAllowDrag', 'draggable', 'arrastavel', 'arrastável', 'permitir_arrastar', 'permitirArrastar'], DEFAULT_APPEARANCE.allow_drag), DEFAULT_APPEARANCE.allow_drag);
    var allowClose = toBoolean(getValue(['allow_close', 'allowClose', 'floating_allow_close', 'floatingAllowClose', 'close_button', 'closeButton', 'floating_close_button', 'floatingCloseButton', 'permitir_fechar', 'permitirFechar'], DEFAULT_APPEARANCE.allow_close), DEFAULT_APPEARANCE.allow_close);
    ...
    return {
      ...
      width: px(widthNumber), height: px(heightNumber), borderWidth: borderWidth, radius: radius,
      innerRadius: innerRadius, zIndex: zIndexNumber, alignItems: alignItems, objectFit: objectFit,
      showPlayButton: showPlayButton, allowDrag: allowDrag, allowClose: allowClose,
      floating_position: position, floating_shape: shape, floating_width: widthNumber,
      floating_height: heightNumber, floating_object_fit: objectFit, object_fit: objectFit,
      show_play_button: showPlayButton, allow_drag: allowDrag, allow_close: allowClose
    };
  }

  function getPrimaryColor(appearance) { return readAppearanceValue(appearance, ['primary_color', 'primaryColor', 'cor_primaria', 'corPrimaria']) || DEFAULT_APPEARANCE.primary_color; }
  function getSecondaryColor(appearance) { return readAppearanceValue(appearance, ['secondary_color', 'secondaryColor', 'cor_secundaria', 'corSecundaria']) || DEFAULT_APPEARANCE.secondary_color; }
  function getBorderColor(appearance) { return readAppearanceValue(appearance, ['floating_border_color', 'floatingBorderColor', 'floating_video_border_color', 'floatingVideoBorderColor', 'border_color', 'borderColor', 'border-color', 'widget_border_color', 'widgetBorderColor', 'cor_borda', 'corDaBorda', 'cor_da_borda']); }
  function getBackgroundColor(appearance) { return readAppearanceValue(appearance, ['background_color', 'backgroundColor', 'bg_color', 'bgColor', 'modal_background', 'modalBackground', 'modal_background_color', 'modalBackgroundColor', 'cor_fundo']) || '#ffffff'; }
  function getButtonColor(appearance) { return readAppearanceValue(appearance, ['button_color', 'buttonColor', 'btn_color', 'btnColor', 'cta_color', 'ctaColor', 'cor_botao']) || getPrimaryColor(appearance); }
  function getTextColor(appearance) { return readAppearanceValue(appearance, ['text_color', 'textColor', 'cor_texto']) || DEFAULT_APPEARANCE.text_color; }
  ...
  function normalizeModalAppearanceConfig(appearance) {
    ...
    return {
      ...
      hide_stories: toBoolean(firstDefined(appearance.hide_stories, appearance.hideStories), DEFAULT_APPEARANCE.hide_stories),
      shadow_enabled: toBoolean(firstDefined(appearance.shadow_enabled, appearance.shadowEnabled), DEFAULT_APPEARANCE.shadow_enabled)
    };
  }
  ...
  function matchesRule(rule) {
    ...
    switch (rule.condition_type) {
      ...
      case 'product_pages': return path.indexOf('/product') !== -1 || path.indexOf('/products') !== -1 || path.indexOf('/produto') !== -1 || path.indexOf('/produtos') !== -1;
      case 'category_pages': return path.indexOf('/category') !== -1 || path.indexOf('/categories') !== -1 || path.indexOf('/categoria') !== -1 || path.indexOf('/colecao') !== -1 || path.indexOf('/collections') !== -1;
      case 'contains': return href.indexOf(value) !== -1;
      case 'equals': return href === value;
      case 'starts_with': return href.indexOf(value) === 0;
      case 'ends_with': return href.lastIndexOf(value) === href.length - value.length;
  ...
  function getVideoUrl(video) {
    if (!video) return '';
    return normalizeMediaUrl(firstDefined(video.video_url, video.videoUrl, video.url, video.source_url, video.sourceUrl, video.external_url, video.externalUrl, video.file_url, video.fileUrl, video.public_url, video.publicUrl, video.video, video.src, ''));
  }
  ...
  function getThumbnailFromObject(obj) {
    ...
    var direct = firstDefined(obj.thumbnail_url, obj.thumbnailUrl, obj.thumbnail, obj.thumb_url, obj.thumbUrl, obj.thumb, obj.poster_url, obj.posterUrl, obj.poster, obj.cover_url, obj.coverUrl, obj.cover, obj.capa_url, obj.capaUrl, obj.capa, obj.image_url, obj.imageUrl, obj.image, obj.photo_url, obj.photoUrl, obj.picture_url, obj.pictureUrl, obj.preview_url, obj.previewUrl, meta.thumbnail_url, meta.thumbnailUrl, meta.thumbnail, meta.thumb_url, meta.thumbUrl, meta.thumb, meta.poster_url, meta.posterUrl, meta.poster, meta.cover_url, meta.coverUrl, meta.cover, meta.capa_url, meta.capaUrl, meta.capa, meta.image_url, meta.imageUrl, meta.image, '');
  ...
  function getVideoThumbnail(video) {
    ...
    if (video.video && typeof video.video === 'object') { direct = getThumbnailFromObject(video.video); if (direct) return direct; }
    if (video.media && typeof video.media === 'object') { direct = getThumbnailFromObject(video.media); if (direct) return direct; }
    if (video.file && typeof video.file === 'object') { direct = getThumbnailFromObject(video.file); if (direct) return direct; }
  ...
  // NOVA FUNÇÃO: Gerencia a fila global de stories e avanço automático
  function openStory(storiesList, initialStoryIndex, storyVideoMap, activeVideos, storyProducts, products) {
    ...
    function getOrderedVideos(s) {
      var relations = (storyVideoMap.get(s.id) || []).slice().sort(function(a,b) {
        return Number(a.position || 0) - Number(b.position || 0);
      });
      return relations.map(function(rel) {
        return activeVideos.find(function(v) { return idsEqual(v.id, rel.video_id); });
      }).filter(Boolean);
    }

    // Identifica o vídeo correto (capa) para iniciar o modal
    var initialStory = storiesList[currentStoryIndex];
    var initialRels = (storyVideoMap.get(initialStory.id) || []).slice().sort(function(a,b) {
      return Number(a.position || 0) - Number(b.position || 0);
    });
    initialRels.forEach(function (rel, index) {
      if (rel.is_cover) currentVideoIndex = index;
    });
    ...
    function renderCurrent() {
      ...
      var isFirstGlobal = currentStoryIndex === 0 && currentVideoIndex === 0;
      prevBtn.disabled = isFirstGlobal;
      if (isFirstGlobal) prevBtn.style.opacity = '0.5';
      prevBtn.addEventListener('click', prevVideo);

      var nextBtn = createEl('button', 'vl-btn vl-btn-primary');
      nextBtn.type = 'button';
      var isLastGlobal = currentStoryIndex === storiesList.length - 1 && currentVideoIndex === orderedVideos.length - 1;
      nextBtn.textContent = isLastGlobal ? 'Fechar' : 'Próximo';
  ...
  function setupFloatingDrag(host, handle) {
    ...
    function stop() {
      dragging = false;
      try { if (activePointerId !== null) handle.releasePointerCapture(activePointerId); } catch (e) {}
      activePointerId = null;
      // Reseta drag para não bloquear cliques justos
      setTimeout(function() {
  ...
  function renderFloatingBubbles(stories, storyVideoMap, activeVideos) {
    ...
    stories.forEach(function (story, index) {
      ...
      // Tenta renderizar um mini-player de vídeo para widget flutuante mais realista
      if ((isDirect || isUpload) && videoUrl) {
        ...
        setImportant(vidPreview, 'border-radius', floatingCfg.innerRadius);
        setImportant(vidPreview, 'object-fit', behaviorConfig.objectFit);
        setImportant(vidPreview, 'overflow', 'hidden');

        var playPromise = vidPreview.play();
        if (playPromise !== undefined) {
          playPromise.catch(function() {
             // Fallback pra imagem se autoplay for bloqueado
             if (thumb) {
               var fbImg = createEl('img', 'vl-img');
               fbImg.src = thumb;
               setImportant(fbImg, 'border-radius', floatingCfg.innerRadius);
               setImportant(fbImg, 'object-fit', behaviorConfig.objectFit);
               inner.innerHTML = '';
  ...
  function renderCarousel(stories, storyVideoMap, activeVideos) {
    ...
    stories.forEach(function (story, index) {
      ...
      if (thumb) {
        ...
        img.alt = story.title || story.name || 'Story';
        img.loading = 'lazy';
        img.onerror = function () { mediaBox.innerHTML = ''; mediaBox.textContent = (story.title || story.name || 'S').slice(0, 1).toUpperCase(); };
      ...
      trackMetric({ event_type: 'impression', story_id: story.id, video_id: coverVideo ? coverVideo.id : null, page_url: window.location.href });
    });
  ...
  function renderWidget() {
    return Promise.all([
      readAppearance(), readStories(), readStoryVideos(), readVideos(), readStoryProducts(), readProducts(), readPageRules()
      ...
      storyVideos.forEach(function (item) {
        ...
        if (!storyVideoMap.has(storyId)) storyVideoMap.set(storyId, []);
      ...
      var storiesWithVideos = stories.filter(function (story) {
        ...
        return rels.some(function (rel) {
          return activeVideos.some(function (video) { return idsEqual(video.id, rel.video_id); });
        });
      });
  ...
  function initMutationObserver() {
    ...
    var observer = new MutationObserver(function (mutations) {
      ...
      mutations.forEach(function (mutation) {
        ...
        if (mutation.type === 'attributes' && mutation.target && mutation.target.id === 'vidlytics-widget-root') shouldForce = true;
      });
