...
const normalizeSettingsFromRow = (row: any): AppSettings => {
  ...
  const merged = {
    ...
    created_at: typeof row.created_at === "string" ? row.created_at : DEFAULT_SETTINGS.created_at,
    updated_at: typeof row.updated_at === "string" ? row.updated_at : DEFAULT_SETTINGS.updated_at,
  };
...
const buildSettingsJson = (value: AppSettings) => {
  const { id, created_at, updated_at, ...settingsJson } = value;
  return {
    ...settingsJson,
...
const SettingsPage = () => {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        if (data?.length) {
          const loaded = normalizeSettingsFromRow(data[0]);
          setSettings(loaded);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const settingsToSave: AppSettings = { ...settings, store_logo_url: finalLogoUrl, updated_at: now };
    ...
    <div className="space-y-8 animate-fade-in pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">Configure dados da loja, módulos, integrações e comportamento dos vídeos.</p>
        </div>
      </div>
          ...
          <CardContent className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                <Input type="text" placeholder="Nome da sua loja" value={settings?.store_name ?? ''} onChange={(e) => setSettings(prev => ({ ...prev, store_name: e.target.value }))} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold outline-none focus:border-[#0094EB]" required />
              </div>
              <div className="space-y-4">
                <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">URL da Loja *</Label>
                <Input type="url" placeholder="https://sualoja.com" value={settings?.store_url ?? ''} onChange={(e) => setSettings(prev => ({ ...prev, store_url: e.target.value }))} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold outline-none focus:border-[#0094EB]" required />
              </div>
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-4">
                    <div className="h-20 w-20 rounded-2xl overflow-hidden bg-slate-200 border border-slate-300 flex items-center justify-center">
                      {logoPreview ? <img src={logoPreview} className="w-full h-full object-cover" alt="Logo" /> : <Image className="w-8 h-8 text-slate-400" />}
                    </div>
                    <div className="flex-1 space-y-2">
                      <div className="flex gap-2">
                        <Input type="file" accept="image/jpeg,image/png,image/webp" onChange={handleLogoChange} className="flex-1 text-xs text-slate-500 file:mr-2 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:bg-[#EAF6FF] file:text-[#0094EB] file:font-bold file:cursor-pointer hover:file:bg-[#0094EB] hover:file:text-white transition-all" />
                        <Button variant="outline" size="icon" onClick={handleRemoveLogo}><X size={20} className="text-rose-600" /></Button>
                      </div>
                      {logoPreview && <p className="text-xs text-slate-500 mt-1">{(new URL(logoPreview)).pathname.split('/').pop()}</p>}
                    </div>
                  </div>
                </div>
              </div>
              <div className="space-y-4">
                ...
                <Input type="email" placeholder="contato@sualoja.com" value={settings?.contact_email ?? ''} onChange={(e) => setSettings(prev => ({ ...prev, contact_email: e.target.value }))} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold outline-none focus:border-[#0094EB]" />
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-none shadow-sm rounded-[2.5rem] overflow-hidden bg-white">
          ...
          <CardContent className="p-6 space-y-6">
            <div className="space-y-4">
              ...
              <Input type="tel" placeholder="5545998888888" value={settings?.whatsapp_number ?? ''} onChange={(e) => { const v = e.target.value.replace(/[^\d+\-\(\) ]/g, ''); setSettings(prev => ({ ...prev, whatsapp_number: v })); }} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold outline-none focus:border-[#0094EB]" />
            ...
            <div className="space-y-4">
              ...
              <Textarea value={settings?.whatsapp_message_template ?? ''} onChange={(e) => setSettings(prev => ({ ...prev, whatsapp_message_template: e.target.value }))} rows={4} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold outline-none focus:border-[#0094EB]" />
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6 space-y-6">
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                <Input type="text" value={settings?.public_live_key ?? ''} readOnly className="flex-1 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-mono text-slate-600 break-all" />
                ...
                <Button variant="outline" size="icon" onClick={() => { const k = 'pub_live_' + Math.random().toString(36).substr(2, 24); setSettings(prev => ({ ...prev, public_live_key: k })); toast.success('Token regenerado'); }}><RefreshCw size={20} className="text-amber-600" /></Button>
              </div>
            </div>
          </CardContent>
        </Card>
