...
const StoriesPage = () => {
  ...
  const [filterSource, setFilterSource] = useState<
    'all' | 'upload' | 'instagram' | 'tiktok' | 'external_url'
  >('all');
  ...
  const processedVideos = useMemo(() => {
    return videos
      .filter((video) => {
        ...
        const matchesSource =
          filterSource === 'all' || video.source_type === filterSource;

        const matchesProduct =
          productFilter === 'all' ||
          (video as Video & { product_id?: string }).product_id ===
            productFilter;
      ...
      .map((video) => {
        ...
        for (let i = 0; i < days; i++) {
          ...
          const dailyLikes = Math.floor(
            dailyViews * (0.1 + (daySeed % 15) / 100)
          );

          const dailyComments = Math.floor(
            dailyLikes * (0.05 + (daySeed % 5) / 100)
          );
      ...
      .sort((a, b) => {
        switch (orderBy) {
          ...
          case 'comments_asc':
            return a.metrics.comments - b.metrics.comments;

          case 'engagement_desc':
            return b.metrics.engagement - a.metrics.engagement;

          case 'engagement_asc':
            return a.metrics.engagement - b.metrics.engagement;

          default:
            return 0;
        }
      });
  }, [videos, searchTerm, filterSource, productFilter, orderBy]);
  ...
  const handleDeleteClick = (video: Video) => {
    const checkUsedInStories = async () => {
      try {
        ...
        const isUsed = storyVideos.some((storyVideo) => {
          return storyVideo.video_id === video.id;
        });
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      setVideos((prev) =>
        prev.filter((video) => video.id !== deleteModal.videoId)
      );
          ...
          const productName =
            products.find(
              (product) =>
                product.id ===
                (video as Video & { product_id?: string }).product_id
            )?.name || 'Produto não vinculado';

          return (
            <div
              key={video.id}
              className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm group"
            >
              <div
              ...
              <div className="p-4">
                <h4 className="font-bold text-slate-800 truncate text-sm mb-3">
                  {video.title}
                </h4>
              ...
              {viewingVideo.video_url ? (
                ...
                <div className="aspect-[9/16] bg-slate-950 rounded-[1.5rem] overflow-hidden shadow-lg relative border-[4px] border-slate-900 max-h-[60vh] flex flex-col items-center justify-center gap-4 p-4">
                  <p className="text-white text-sm font-bold text-center">
                    {viewingVideo.source_type === 'instagram'
                      ? 'Vídeo do Instagram'
                      : viewingVideo.source_type === 'tiktok'
                        ? 'Vídeo do TikTok'
                        : 'Sem vídeo'}
                  </p>
                  ...
                  {viewingVideo.instagram_link && (
                    <a
                      href={viewingVideo.instagram_link}
                      target="_blank"
                      rel="noreferrer"
                      className="bg-[#0094EB] text-white px-4 py-2 rounded-xl text-xs font-black"
                    >
                      Abrir no Instagram
                    </a>
                  )}
