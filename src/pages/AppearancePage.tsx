...
return (
    <div className="overflow-hidden rounded-[1rem] border border-slate-200 bg-slate-50 flex flex-col h-full min-h-[480px]">
      {/* Mockup Header E-commerce */}
      <div className="flex items-center justify-between bg-white px-4 py-3 shadow-sm shrink-0">
        <div className="flex items-center gap-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg" style={{ backgroundColor: colors.primary }}>
            <span className="text-[10px] font-black text-white">LJ</span>
          </div>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden [&::-webkit-scrollbar]:hidden">
        ...
        {/* Carrossel do Vidlytics inserido no site */}
        <div
          className="mt-6"
          style={{
            marginTop: cssSize(carousel.margin_top, '24px'),
            marginBottom: cssSize(carousel.margin_bottom, '24px'),
          }}
        >
          {carousel.show_title && (
            <div className="px-4 mb-3">
              <div className="h-3.5 w-32 rounded bg-slate-800" style={{ opacity: 0.8 }} />
            </div>
          )}
          <div className="pl-4">
            <div
              className={cn(
                'flex overflow-x-auto pb-4 pr-4 [&::-webkit-scrollbar]:hidden',
                carousel.auto_center && 'justify-center pr-0 pl-0'
              )}
            ...
            >
              {items.map((_, index) => (
                <div key={index} className="flex flex-col gap-1.5 shrink-0" style={{ width: cardWidth }}>
                  ...
                  >
                    {carousel.show_play_icon && (
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="flex h-7 w-7 items-center justify-center rounded-full bg-white/90 shadow-sm" style={{ color: colors.button }}>
                          <PlaySquare size={14} />
                        </div>
                      </div>
                    )}
                  ...
                  {/* Card de Produto Realista (Lado a Lado) */}
                  {carousel.show_product && !isCircle && (
                    <div
                      className="flex items-center gap-1.5 p-1.5 shadow-sm bg-white"
                      style={{
                    ...
                    >
                      <div className="h-8 w-8 shrink-0 rounded bg-slate-200" />
        ...
        {/* Mockup Grid de Produtos Falsos abaixo do Carrossel */}
        <div className="px-4 mt-2 pb-8">
          <div className="h-3.5 w-40 rounded bg-slate-800 mb-4" style={{ opacity: 0.8 }} />
          <div className="grid grid-cols-2 gap-3">
            <div className="h-36 rounded-xl bg-white shadow-sm border border-slate-100" />
            <div className="h-36 rounded-xl bg-white shadow-sm border border-slate-100" />
            <div className="h-36 rounded-xl bg-white shadow-sm border border-slate-100" />
            <div className="h-36 rounded-xl bg-white shadow-sm border border-slate-100" />
          </div>
  ...
  return (
    <div className="overflow-hidden rounded-[1.25rem] border border-slate-200 bg-slate-100 p-4">
      <div className="rounded-[1rem] border border-slate-200 bg-white p-3">
        <div
          className="relative mx-auto h-[380px] max-w-[230px] overflow-hidden rounded-[1.5rem]"
          style={{
            background: `linear-gradient(160deg, ${colors.primary}, ${colors.secondary})`,
            color: '#FFFFFF',
...
{/* Modal de edição */}
      {showModal && (
        <div className="fixed inset-0 z-[999999] flex items-center justify-center bg-slate-900/60 p-3 xl:pl-[270px] backdrop-blur-sm">
          <div className="flex h-[95vh] w-full max-w-[calc(100vw-290px)] flex-col overflow-hidden rounded-[1.5rem] bg-white shadow-2xl">
