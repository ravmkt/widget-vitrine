  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F7FAFC] p-4">
      <form onSubmit={handleSubmit} className="w-full max-w-md rounded-[2rem] bg-white p-8 shadow-sm border border-slate-200 space-y-4">
        ...
        <input className="w-full rounded-xl border border-slate-200 px-4 py-3" placeholder="Senha (mínimo 6 caracteres)" type="password" value={password} onChange={(e) => setPassword(e.target.value)} required />
        <input className="w-full rounded-xl border border-slate-200 px-4 py-3" placeholder="Confirme a senha" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required />
        <input className="w-full rounded-xl border border-slate-200 px-4 py-3" placeholder="Nome da empresa/loja" value={storeName} onChange={(e) => setStoreName(e.target.value)} required />
        {error && <p className="text-sm font-bold text-rose-600">{error}</p>}
        <button disabled={loading || googleLoading} className="w-full rounded-xl bg-[#0094EB] py-3 font-black text-white hover:brightness-95 transition-all disabled:opacity-50">{loading ? 'Criando...' : 'Criar conta'}</button>
