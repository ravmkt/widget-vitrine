...
const StoryDetailsPage = () => {
  ...
  const handleSave = async (event: FormEvent) => {
    ...
    try {
      ...
      const newRelations: StoryVideo[] = validSelectedVideoIds.map((videoId, index) => ({ id: generateUuid(), store_id: finalStoreId, story_id: savedStory.id, video_id: videoId, position: index + 1, is_cover: index === 0, created_at: now }));
            ...
            <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="space-y-4">
                <div className="grid gap-4 md:grid-cols-[220px_1fr] md:items-end">
                  ...
                  <div className="space-y-2"><label className="text-[9px] font-black uppercase tracking-widest text-slate-400">VALOR</label><input type="text" className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold outline-none" placeholder="/colecao, /produto/nome-do-produto ou trecho da URL" /></div>
                </div>
                ...
                {pageRules.map((rule) => (
                  <div key={rule.id} className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                    <div className="grid gap-4 md:grid-cols-[220px_1fr_auto] md:items-end">
                      ...
                      <div className="space-y-2"><label className="text-[9px] font-black uppercase tracking-widest text-slate-400">VALOR</label>{CONDITION_TYPES_WITH_VALUE.includes(rule.condition_type) ? <input type="text" value={rule.value} onChange={(event) => handleUpdatePageRule(rule.id, { value: event.target.value })} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold outline-none" placeholder="/colecao, /produto/nome-do-produto ou trecho da URL" /> : <div className="h-[34px] rounded-xl border border-dashed border-slate-200 bg-slate-50" />}</div>
                      <div className="flex items-end justify-end">{pageRules.length > 1 && <button type="button" onClick={() => handleDeletePageRule(rule.id)} className="rounded-2xl bg-rose-500 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-white shadow-md shadow-rose-100 transition-all hover:bg-rose-600 hover:shadow-lg">Remover</button>}</div>
