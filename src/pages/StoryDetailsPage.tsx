                ...
                {pageRules.map((rule) => (
                  <div key={rule.id} className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                    <div className="grid gap-4 md:grid-cols-[220px_1fr_auto] md:items-end">
                      ...
                      <div className="flex items-end justify-end">{pageRules.length > 1 && <button type="button" onClick={() => handleDeletePageRule(rule.id)} className="rounded-xl border border-rose-200 bg-white px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-rose-500 hover:bg-rose-50">Remover</button>}</div>
                    </div>
                  </div>
                ))}
