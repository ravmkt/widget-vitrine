...
import {
  ...
  MessageCircle, Send, Heart, ShoppingBag, Bookmark, BookmarkPlus, Trash2, Check, Sparkles
} from "lucide-react";
...
const DEFAULT_TEMPLATES: LiveAppearanceTemplate[] = [
  {
    id: "preset_padrao",
  ...
  },
  {
    ...
    name: "Black Friday (Dourado & Preto)",
    isCustom: false,
    divulgacao: {
  ...
  {
    id: "preset_cyber",
    name: "Cyber Purple (Roxo Neon)",
    isCustom: false,
    divulgacao: {
      desktop: { ...defaultDivulgacaoSettings, borderColor: "#8b5cf6", ctaBgColor: "#8b5cf6", countdownBgColor: "#8b5cf6" },
      mobile: { ...defaultDivulgacaoSettings, width: 80, borderColor: "#8b5cf6", ctaBgColor: "#8b5cf6", countdownBgColor: "#8b5cf6" },
      linked: false
    },
    aoVivo: {
      desktop: { ...defaultAoVivoSettings, borderColor: "#8b5cf6", ctaBgColor: "#8b5cf6" },
      mobile: { ...defaultAoVivoSettings, width: 80, borderColor: "#8b5cf6", ctaBgColor: "#8b5cf6" },
      linked: false
    },
    player: {
      desktop: {
...
const STORAGE_KEY_TEMPLATES = "vidlytics_live_appearance_templates";
...
export default function LiveAppearanceModal({ isOpen, onClose, onSave, isSaving }: Props) {
  ...
  const [templates, setTemplates] = useState<LiveAppearanceTemplate[]>(DEFAULT_TEMPLATES);
  const [selectedTemplateId, setSelectedTemplateId] = useState<string>("preset_padrao");
  const [isTemplateModalOpen, setIsTemplateModalOpen] = useState(false);
  const [newTemplateName, setNewTemplateName] = useState("");
  const [feedbackMsg, setFeedbackMsg] = useState("");

  // Carrega templates salvos do localStorage
  useEffect(() => {
    try {
  ...
  const handleApplyTemplate = (templateId: string) => {
    ...
    setFeedbackMsg(`Template "${tpl.name}" aplicado!`);
  ...
  const handleSaveAsTemplate = () => {
    ...
    const updatedCustoms = [...customOnly, newTemplate];
    
    try {
      localStorage.setItem(STORAGE_KEY_TEMPLATES, JSON.stringify(updatedCustoms));
    } catch (e) {
    ...
    setFeedbackMsg(`Template "${newTemplate.name}" gravado com sucesso!`);
    setTimeout(() => setFeedbackMsg(""), 3000);
  };
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-[97vw] w-[97vw] h-[95vh] p-0 flex flex-col overflow-hidden bg-white shadow-2xl rounded-2xl border-0">
          ...
          <div className="px-5 py-2.5 border-b border-slate-200 bg-white flex items-center justify-between shrink-0 z-20">
            ...
            {/* SELEÇÃO E GRAVAÇÃO DE TEMPLATES */}
            <div className="flex items-center gap-2">
              {feedbackMsg && (
                <div className="flex items-center gap-1 text-[11px] text-emerald-700 bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-full animate-in fade-in">
                  <Check className="w-3 h-3 text-emerald-600" />
                  <span>{feedbackMsg}</span>
                </div>
              )}

              <div className="flex items-center gap-1 bg-slate-50 border border-slate-200 p-1 rounded-xl shadow-sm">
                <Bookmark className="w-3.5 h-3.5 text-slate-400 ml-1.5 shrink-0" />
                <span className="text-[11px] font-bold text-slate-500 uppercase tracking-tight pl-1">Template:</span>
                <select
                  value={selectedTemplateId}
                  onChange={(e) => handleApplyTemplate(e.target.value)}
                  className="bg-transparent text-[11.5px] font-semibold text-slate-800 outline-none cursor-pointer pr-1"
                >
                  <optgroup label="Padrões do Vidlytics">
                ...
                {templates.find(t => t.id === selectedTemplateId)?.isCustom && (
                  <button
                ...
                <Button
                  ...
                  variant="ghost"
                  className="h-6 px-2 text-[11px] font-bold text-rose-600 hover:bg-rose-50 rounded-lg flex items-center gap-1"
                >
                  <BookmarkPlus className="w-3.5 h-3.5" /> Gravar Atual
                </Button>
              </div>

              <button onClick={onClose} className="p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-full transition-colors ml-1">
                ✕
              </button>
            </div>
          </div>
          ...
          <div className="px-5 py-2.5 border-t border-slate-200 bg-white flex items-center justify-between shrink-0 z-20">
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={handleSave} disabled={isSaving} className="bg-rose-600 hover:bg-rose-700 text-white shadow-sm h-8 px-5 text-[12px] font-medium rounded-full">
                <Save className="h-3.5 w-3.5 mr-1.5" /> {isSaving ? "Salvando..." : "Salvar"}
              </Button>
            </div>
          </div>
      ...
      {/* MODAL DE GRAVAÇÃO DE NOVO TEMPLATE */}
      <Dialog open={isTemplateModalOpen} onOpenChange={setIsTemplateModalOpen}>
        <DialogContent className="max-w-[420px] p-5 bg-white rounded-2xl shadow-2xl border border-slate-200">
          <div className="space-y-4">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="text-[15px] font-bold text-slate-800">Gravar Novo Template</h3>
                <p className="text-[11.5px] text-slate-500">Salve as cores e estilos atuais para reutilizar quando quiser.</p>
              </div>
            </div>

            <div className="space-y-1.5">
              <label className="text-[11px] font-bold text-slate-600 uppercase tracking-wide">Nome do Template</label>
              <Input
                placeholder="Ex: Black Friday 2026, Dia das Mães, Verão..."
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button
                ...
                className="h-8 text-[12px] bg-rose-600 hover:bg-rose-700 text-white rounded-lg px-4"
              >
                Gravar Template
              </Button>
            </div>
