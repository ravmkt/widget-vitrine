...
{device === 'mobile' ? (
  ...
  // ==========================================================
  <div className="w-full h-full flex items-center justify-center gap-4 sm:gap-6 overflow-x-auto px-2">

    {/* Componente reutilizável do celular base (topo, coluna direita, cupom) */}
    {[
      { id: "normal",   label: "Player" },
      { id: "products", label: "Com Produtos" },
      { id: "chat",     label: "Com Chat" },
    ].map((phone) => (
      <div key={phone.id} className="flex flex-col items-center gap-2 shrink-0">
        <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">{phone.label}</span>

        <div className="relative h-full max-h-[560px] aspect-[9/19] bg-[#0a0a0a] rounded-[2.5rem] shadow-[0_0_0_3px_#f4d1c0] border-[6px] border-[#0a0a0a] overflow-hidden flex flex-col shrink-0 select-none">
          ...
          <div className="absolute top-5 left-3.5 flex items-center gap-2 z-20">
            <div className="w-7 h-7 rounded-full bg-[#1b4332] border border-white/40 flex items-center justify-center text-[8px] font-bold text-white shadow shrink-0">USE</div>
            {currentPlayer.showTitle && (
          ...
          <div className="absolute top-5 right-3 flex flex-col items-center gap-2.5 z-20">
            ...
            {currentPlayer.showChat && (
              <button className={`w-7 h-7 rounded-full flex items-center justify-center text-white border shadow-sm transition ${phone.id === 'chat' ? 'bg-white/20 border-white/60' : 'bg-black/45 border-white/20'}`}>
            ...
            {currentPlayer.showProducts && (
              <div className="flex flex-col items-center -mt-0.5">
                <button className={`w-7 h-7 rounded-full flex items-center justify-center text-white border shadow-sm transition ${phone.id === 'products' ? 'bg-white/20 border-white/60' : 'bg-black/45 border-white/20'}`}>
                  <ShoppingCart className="w-3.5 h-3.5"/>
                </button>
                <span className="text-white text-[8px] font-bold drop-shadow mt-0.5">3</span>
              </div>
            )}
            <button className="w-7 h-7 bg-[#25D366] rounded-full flex items-center justify-center text-white border border-white shadow-md">
              ...
              <div className="flex flex-col gap-1.5 mb-2 overflow-hidden" style={{ maskImage: 'linear-gradient(to top, black 70%, transparent 100%)', WebkitMaskImage: 'linear-gradient(to top, black 70%, transparent 100%)' }}>
                {[1,2,3].map(i => (
                  <div key={i} className="flex items-center gap-1.5 drop-shadow">
                    ...
                    <span className="text-white text-[9px] font-medium drop-shadow leading-none">Nononononono</span>
                  </div>
                ))}
              </div>
            ...
            <div className="absolute bottom-2.5 left-2.5 right-2.5 z-20">
              <div className="bg-white rounded-[12px] p-1.5 flex gap-2 items-center shadow-lg border" style={{ borderColor: currentPlayer.borderColor }}>
                ...
                <div className="flex-1 min-w-0 pr-1">
                  <div className="text-[10px] font-bold leading-tight line-clamp-2" style={{ color: currentPlayer.productNameColor }}>
                    Blusa Life Rosê em Malha Tecnológica
                  </div>
                  <div className="text-[8px] text-slate-400 line-through mt-0.5 leading-none">De: R$ 199,90</div>
          ...
          {phone.id === 'products' && currentPlayer.showProducts && (
            <div className="absolute inset-x-1.5 bottom-1.5 top-6 z-50 flex flex-col overflow-hidden">
              <div className="bg-white flex-1 rounded-2xl shadow-2xl flex flex-col border border-slate-200 overflow-hidden">
                <div className="bg-[#0ea5e9] text-white text-center py-2.5 font-bold text-[13px] tracking-wide shadow-sm">PRODUTOS</div>
                  ...
                  {[1,2,3,4].map(i => (
                    <div key={i} className="bg-white border border-[#0ea5e9]/30 rounded-lg p-1.5 flex gap-2 items-center shadow-sm">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-[10px] font-bold leading-tight line-clamp-1">Blusa Life Rosê</div>
                        <div className="text-[8px] text-slate-400 line-through">De: R$ 199,90</div>
                ...
                <div className="bg-white p-2.5 border-t flex flex-col items-center">
                  <ChevronDown className="w-5 h-5 text-[#0ea5e9] mb-1"/>
  ...
  // ... mantém igual o bloco desktop existente
                      // ----------------------------------------------------
